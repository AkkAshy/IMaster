...
class MonoblokCharSerializer(serializers.ModelSerializer):
    author = UserSerializer(read_only=True)
    specification = MonoblokSpecificationSerializer(read_only=True)  # Добавь это поле
...
class EquipmentSerializer(serializers.ModelSerializer):
    ...
    monoblok_specification_data = MonoblokSpecificationSerializer(source='monoblok_char.specification', read_only=True, allow_null=True)
        ...
        for i in range(count):
            ...
            if equipment_type_name == 'компьютер' and computer_details_data:
                ...
                MonoblokChar.objects.create(
                    equipment=equipment,
                    specification = monoblok_spec,
                    **monoblok_char_data)
                if monoblok_spec:
                   for disk_spec in monoblok_spec.disks.all():
                       Disk.objects.create(equipment=equipment, disk_type=disk_spec.disk_type, capacity_gb=disk_spec.capacity_gb, author=author)
