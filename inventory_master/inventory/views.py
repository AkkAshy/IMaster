...
from .models import (EquipmentType, ContractDocument, Equipment,
                    ...
                    MonitorChar, MonitorSpecification, ContractTemplate, INNTemplate
)
...
from .serializers import (
    ...
    CustomEquipmentSerializer, MonitorCharSerializer, MonitorSpecificationSerializer, ContractTemplateSerializer,
...
class ContractDocumentViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        old_file = instance.file.url if instance.file else None
        new_data = serializer.validated_data
        new_file = new_data.get('file', instance.file)
        serializer.save()
...
class EquipmentViewSet(viewsets.ModelViewSet):
    ...
    # Настройка фильтрации - ИНН только для точного поиска
    filter_backends = [DjangoFilterBackend, SearchFilter, OrderingFilter]
    search_fields = ['name', 'description']  # ИНН убран из search_fields
    filterset_fields = ['type', 'status', 'room', 'inn']  # ИНН остается для точной фильтрации
    ordering_fields = ['created_at', 'name', 'inn']
    ordering = ['-created_at']
    ...
    # Поиск по началу ИНН
    @action(detail=False, methods=['get'], url_path='find-by-inn/(?P<inn>\\d+)')
    def find_by_inn(self, request, inn=None):
        """
        Поиск оборудования по началу ИНН (все ИНН, начинающиеся с введенных цифр)
        """
        if not inn:
            return Response(
        ...
        try:
            inn_str = str(int(inn))  # Убираем ведущие нули и преобразуем обратно
        except ValueError:
            return Response(
        ...
        if not equipments.exists():
            return Response(
                {"error": f"Оборудование с ИНН, начинающимся на {inn_str}, не найдено"},
    ...
    # Поиск по началу ИНН через query параметры
    ...
    def search_by_inn_prefix(self, request):
        ...
        if not inn_prefix and not exact_inn:
            return Response(
                {"error": "Укажите параметр inn_prefix для поиска по началу или exact_inn для точного поиска"},
                status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if equipments.exists():
            ...
            return Response({
                "count": equipments.count(),
                "search_type": "prefix" if inn_prefix else "exact",
                "search_value": inn_prefix or exact_inn,
                "results": serializer.data
            })
        else:
            return Response(
    ...
    def bulk_update_inn(self, request):
        ...
        # Парсим equipments, если это строка JSON
        if 'equipments' in data and isinstance(data['equipments'], str):
            try:
                parsed_equipments = json.loads(data['equipments'])
                if not isinstance(parsed_equipments, list):
                    data['equipments'] = [parsed_equipments]  # Преобразуем одиночный объект в список
                else:
                    data['equipments'] = parsed_equipments  # Оставляем список как есть
                print("Parsed equipments:", data['equipments'])
            except json.JSONDecodeError:
                return Response(
                    {"equipments": "Недопустимый формат JSON в поле equipments"},
                    status=status.HTTP_400_BAD_REQUEST
                )

        serializer = BulkEquipmentInnUpdateSerializer(data=data, context={'request': request})
        if serializer.is_valid():
    ...
    # Обновить существующий метод фильтрации
    ...
    def equipment_filter(self, request):
        ...
        inn = request.query_params.get('inn')  # Добавлен поиск по ИНН
        ...
        if inn:  # Поиск по началу ИНН
            try:
                inn_str = str(int(inn))  # Преобразуем в строку без ведущих нулей
                equipments = equipments.filter(inn__startswith=inn_str)  # Поиск по началу
            except ValueError:
                return Response(
                    {"error": "ИНН должен содержать только цифры"},
                    status=status.HTTP_400_BAD_REQUEST
